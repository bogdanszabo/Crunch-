...
(function(jQuery) {
var selectors = []; var check_binded = false;var check_lock = false; var defaults = {interval: 250, force_process: false};var jQuerywindow = jQuery(window); var jQueryprior_appeared;function process() {check_lock = false;for (var index in selectors) {var jQueryappeared = jQuery(selectors[index]).filter(function() {return jQuery(this).is(':appeared');});jQueryappeared.trigger('appear', [jQueryappeared]);if (jQueryprior_appeared) {var jQuerydisappeared = jQueryprior_appeared.not(jQueryappeared);jQuerydisappeared.trigger('disappear', [jQuerydisappeared]);}jQueryprior_appeared = jQueryappeared;}}jQuery.expr[':']['appeared'] = function(element) {var jQueryelement = jQuery(element);if (!jQueryelement.is(':visible')) {return false;} var window_left = jQuerywindow.scrollLeft(); var window_top = jQuerywindow.scrollTop();var offset = jQueryelement.offset();var left = offset.left; var top = offset.top;if (top + jQueryelement.height() >= window_top &&top - (jQueryelement.data('appear-top-offset') || 0) <= window_top + jQuerywindow.height() &&left + jQueryelement.width() >= window_left &&left - (jQueryelement.data('appear-left-offset') || 0) <= window_left + jQuerywindow.width()) {return true;} else {return false;}};jQuery.fn.extend({appear: function(options) {var opts = jQuery.extend({}, defaults, options || {}); var selector = this.selector || this;if (!check_binded) {var on_check = function() {if (check_lock) {return; } check_lock = true;setTimeout(process, opts.interval);};jQuery(window).scroll(on_check).resize(on_check);check_binded = true;} if (opts.force_process) {setTimeout(process, opts.interval);}selectors.push(selector);return jQuery(selector);}});jQuery.extend({force_appear: function() {if (check_binded) {process();return true;};return false;}});})(jQuery);
...
(function(jQuery, window, document) {
  ...
  YTPlayer = {
    ...
    init: function init(node, userOptions) {
      ...
      self.jQuerynode = jQuery(node),
      self.jQuerywindow = jQuery(window);
      self.defaults.events = {
        ...
        onStateChange: function(e) {
          if (e.data === 1) {
            self.jQuerynode.addClass('loaded');
          } else if (e.data === 0 && self.options.repeat) {
            self.player.seekTo(self.options.start);
          }
        }
      }
      self.options = jQuery.extend(true, {}, self.defaults, self.userOptions);
      ...
      self.jQuerywindow.on('resize.YTplayer' + self.ID, function() {
        self.resize(self);
      });
    ...
    pauseOnScroll: function pauseOnScroll() {
      var self = this;
      self.jQuerywindow.on('scroll.YTplayer' + self.ID, function() {
        var state = self.player.getPlayerState();
        if (state === 1) {
          self.player.pauseVideo();
        }
      });
      self.jQuerywindow.scrollStopped(function(){
        var state = self.player.getPlayerState();
        if (state === 2) {
    ...
    createContainerVideo: function createContainerVideo() {
      var self = this;
      var jQueryYTPlayerString = jQuery('<div id="ytplayer-container' + self.ID + '" >\
                                    <div id="' + self.holderID + '" class="ytplayer-player"></div> \
                                    </div> \
                                    <div id="ytplayer-shield"></div>');

      self.jQuerynode.append(jQueryYTPlayerString);
      self.jQueryYTPlayerString = jQueryYTPlayerString;
      jQueryYTPlayerString = null;
    },
    createBackgroundVideo: function createBackgroundVideo() {
      var self = this,
        jQueryYTPlayerString = jQuery('<div id="ytplayer-container' + self.ID + '" class="ytplayer-container background">\
                                    <div id="' + self.holderID + '" class="ytplayer-player"></div>\
                                    </div>\
                                    <div id="ytplayer-shield"></div>');
    ...
    resize: function resize(self) {
      ...
      if (!self.options.fitToBackground) {
        container = self.jQuerynode;
      }
      var width = container.width(),
        pWidth,
        height = container.height(),
        pHeight,
        jQueryYTPlayerPlayer = jQuery('#' + self.holderID);
      if (width / self.options.ratio < height) {
        pWidth = Math.ceil(height * self.options.ratio);
        jQueryYTPlayerPlayer.width(pWidth).height(height).css({
          left: (width - pWidth) / 2,
          top: 0
        });
      } else {
        pHeight = Math.ceil(width / self.options.ratio); // get new player height
        jQueryYTPlayerPlayer.width(width).height(pHeight).css({
          left: 0,
          top: 0
        });
      }
      jQueryYTPlayerPlayer = null;
      container = null;
    },
    ...
    destroy: function destroy() {
      ...
      self.jQuerynode
        ...
        .removeData('ytPlayer')
        .removeClass('loaded');
      self.jQueryYTPlayerString.remove();
      jQuery(window).off('resize.YTplayer' + self.ID);
      jQuery(window).off('scroll.YTplayer' + self.ID);
      self.jQuerybody = null;
      self.jQuerynode = null;
      self.jQueryYTPlayerString = null;
      self.player.destroy();
      self.player = null;
    }
  };
  ...
  jQuery.fn.YTPlayer = function(options) {
    return this.each(function() {
      var el = this;
      jQuery(el).data("yt-init", true);
      var player = Object.create(YTPlayer);
      player.init(el, options);
      jQuery.data(el, "ytPlayer", player);
    });
  };
...
return b&&"matrix"==b.substr(0,6)?b.replace(/^.*\((.*)\)jQuery/g,"jQuery1").replace(/px/g,"").split(/, +/):!1}function n(){var b=a.win;if("zIndex"in b)return b.zIndex();for(;0<b.length&&9!=b[0].nodeType;){var g=b.css("zIndex");if(!isNaN(g)&&0!=g)return parseInt(g);b=b.parent()}return!1}function d(b,g,l){g=b.css(g);b=parseFloat(g);return isNaN(b)?(b=y[g]||0,l=3==b?l?a.win.outerHeight()-a.win.innerHeight():a.win.outerWidth()-a.win.innerWidth():1,a.isie8&&b&&(b+=1),l?b:0):b}function p(b,g,l,c){a._bind(b,g,function(a){a=
